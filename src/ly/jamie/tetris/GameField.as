public class GameField {
	...
	public static var gf:GameField = new GameField(null);
	...
	public function start():Void {
		...
		for(var i:Number=0; i<this.allBlocks.length; i++) {
	...
	public function gameTick():Number {
		...
		//trace("tick " + this.ticks);
		if( this.nextBlock == null ) {
			//this.nextBlock = this.getRandomBlock(new Point(100,0));
		}

		if( this.stillProcessing ) return;
	...
	public function isEmpty(pt:Point):Boolean {
		...
		return not ((arrBitGameField[y] & (1<<x)) != 0 );
	}
	...
	public function isInBounds(pt:Point):Boolean {
		var x:Number = this.normalizeCoordinate(pt.x);
		var y:Number = this.normalizeCoordinate(pt.y);
		return not ( (y<0 || y >= GameField.height) || (x<0 || x >= GameField.width) );
	}
	...
	public function isGameOver(Void):Boolean {
		return gameIsOver;
	}
}
